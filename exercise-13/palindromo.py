...
def palindromo():
    ...
    frase = str(input("Digite uma frase: ")).strip().upper() # strip = elimina os espaços antes e depois e o upper = a frase é convertida para caixa alta
    palavras = frase.split() # gera um vetor da minha frase
    junto = ''.join(palavras) # pega o vetor e junta sem espaços em uma variavel
    inverso = ''
    for letra in range(len(junto) -1, -1, -1): # foi na ultima letra da frase e foi lendo da direita para a esquerda
        inverso += junto[letra]
